...
export class PluginService {
    ...
    getAdditionalData(successFunc, errorFunc) {
        this.httpService.getPapiApiCall(`/addons/installed_addons/${this.addonUUID}`)
            .subscribe(
                (success) => {
    ...
    publishTheme(publishComment: string, additionalData: any, successFunc, errorFunc = null) {
        const body = {
        ...
        this.addonService.postAddonApiCall(this.addonUUID, 'themes', 'publish', body)
            .subscribe(
                (success) => {
                    if (successFunc) { successFunc(success); }
                },
